...
class Education {
  ...
  static async findEducationByEduId({ eduId }) {
    ...
    const foundEducationByUser = await EducationModel.findOne({ id: eduId });
    return foundEducationByUser;
  }

  // document의 ObjectId로 학력정보를 찾아주는 기능을 구현합니다.
  static async findEducationByUser({ userId }) {
    // EducationSchema에서 정의된 사용자 ID를 사용해서 학력정보 document를 찾습니다.
    // userId는 User 문서의 ObjectId인 _id를 ref로 참조합니다. 스키마를 참고하세요.
    const foundEducationByUser = await EducationModel.findOne({ userId });
    return foundEducationByUser;
  }
  ...
  static async update({ userId, fieldToUpdate, newValue }) {
    ...
    // 문서를 찾을 때 사용할 사용자의 ObjectId를 지정합니다.
    ...
    // 위에서 지정한 값에 맞게 학력정보 document를 최신화합니다.
  ...
  static async delete({ eduId }) {
    return EducationModel.findOneAndDelete({ id: eduId });
  }
}
